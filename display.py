...
from settings import CHAR_SIZE, HEIGHT, WIDTH
...
class Display:
    ...
    def show_life(self, life):
        ...
        life_image_scaled = pygame.transform.scale(life_image, (CHAR_SIZE, CHAR_SIZE))
        life_x = CHAR_SIZE // 2
        if life != 0:
            for life in range(life):
                self.screen.blit(life_image_scaled, (life_x, HEIGHT + (CHAR_SIZE // 2)))
                life_x += CHAR_SIZE
